...
@Transactional
@Scope("prototype")
public class DissentFacade implements IDissentFacade{
    
	
	@Autowired
	Reg_Demurrer demurrerDao;
	
	//二元数组,数组值意义,1.数据表里的字段,2.实体对象对应字段,3.类型区分【1:代表字符串类型,2:代表时间类型,3:代表日期加时间,4:代表金额类型】
	static final String[][] dissentOptions = {{"tab.r_coll_names","holdername","1"},
	...
	@Override
	public Page<Map<String, Object>> getAllDissentsByParam(
			Map<String, Object> paramMap) {
		String sqlwhere = null;
		int pageNo = 1;
		int pageSize = 3;
		Map<String,Object> resultMap = SQLUtils.growWhereStatment(Util.ConvertObjToMap(paramMap.get("DXTXOBJECT")), dissentOptions,null);
		...
		return demurrerDao.queryMapPageBykeyForOracle("Demurrer.getAllDemurrerInfos", sqlwhere, fieldMap,pageNo,pageSize);
	}
	...
	public List<Map<String, Object>> getDemurrersBydemurrerId(Map<String,Object> sqlMap,
			   ...
			   wheresql.append(sqlMap.get("demurrerId"));
			   wheresql.append(" ");
			   wheresql.append(sqlMap.get("reg_unit_type"));
			   if(valueLst!=null){
				   retnLst = new ArrayList<Map<String,Object>>();
				   for(int i = 0; i < valueLst.size(); i++){
					   Map<String ,Object> map1 = demurrerDao.queryMapByKey("Demurrer.getAllO", wheresql.toString(), valueLst.get(i));
					   if(map1!=null)
					   retnLst.add(map1);
				   }
			   }
