				...
				for(var i=0; i<properties.length; i++){
					...
					if (typeof pp == 'string'){
						if (pp == 'width' || pp == 'height' || pp == 'left' || pp == 'top'){
							opts[pp] = t.attr(pp) || parseInt(target.style[pp]) || undefined;
						} else {
							opts[pp] = t.attr(pp);
						}
					} else {
...
//				var p = $('<div class="tree-node-proxy tree-dnd-no"></div>').appendTo('body');
			...
			onStartDrag: function(){
				...
				if (node.id == undefined){
					node.id = 'plui_tree_node_id_temp';
			...
			onStopDrag: function(){
				...
				if (node.id == 'plui_tree_node_id_temp'){
					node.id = '';
					updateNode(target, node);
				}
...
//				});
				...
				var nodeitem = $('<span class="tree-title"></span>').html(item.text).appendTo(node);
				//添加title(鼠标移上去显示)
				if (item.tip) {
					nodeitem.attr('title', item.tip);
				}
	...
	function expandNode(target, nodeEl, callback){
		...
		if (ul.length){
			//loadData
			if (ul.html() == '') {
				loadData(target, ul[0], node.children);
				//console.log(opts.query);
				if (opts.query) {
					visible(target, node.children, opts.query);
				}
			}
...
//					getData(nodedata.children, sub);
	...
	function findNode(target, id){
		...
		if (node.length){
			...
			if (id) {
				//如果是根据ID查找节点 - 从节点的children数据中迭代
				//chenlia-2013/09/23
				var rootNodes = getRootNodes(target);
				for (var j = 0; j < rootNodes.length; j ++) {
					var path = findNodeByChildren('', rootNodes[j], id);
					if (path == null) { return null }	//未找到直接返回
					var paths = path.split(',');
					for (var i = 0; i < paths.length; i ++) {
						if (paths[i]) {
							if (paths[i] == id) {
								//返回找到的节点
								return getNode(target, $(target).find('div.tree-node[node-id=' + id + ']')[0]);
							} else {
								//判断节点是否展开，未展开则展开该节点
								expandNode(target, $(target).find('div.tree-node[node-id=' + paths[i] + ']')[0]);
							}
						}
					}
				}
	 ...
	 * 迭代
	 * 根据节点数据获取传入节点的text
	 * 由于设置了lazy，部分节点没构建成dom节点，jquery不能直接find到，需要从数据中分析
	 * chenlia-2013/09/23
	 */
	function findNodeByChildren(path, node, id) {
...
//			return node.text;
			...
			if (node.children) {
				for (var i = 0; i < node.children.length; i ++) {
					ret = findNodeByChildren(path, node.children[i], id);
					if (ret) {
						return ret;
					}
				}
	...
	function query(target, q) {
		...
		//如果是延迟渲染
		//展示所有节点
		$(target).find('div.tree-node').show();
		...
		for (var i = 0; i < roots.length; i ++) {
			//console.log(roots[i]);
			//文本中不包含查询字符串
			if (roots[i].text.indexOf(q) == -1) {
				if (roots[i].children) {
					//存在子节点时 - 判断子节点是否显示
					if (!visible(target, roots[i].children, q)) {
						//如果子节点不显示 - 根节点不显示
						$(roots[i].target).hide();
					}
				} else {
	...
	//判断节点是否显示
	function visible(target, children, q) {
		var ret = false, node;
		//循环根节点
		for (var i = 0; i < children.length; i ++) {
			var tmpret = false;
			...
			//获取子节点是否显示
			if (node.children) {
				tmpret = visible(target, node.children, q);
			}
			...
			if (!tmpret) {
				if (node.text.indexOf(q) == -1) {
					//父节点不显示
					//$(node.target).hide();
					$(target).find('div.tree-node[node-id="' + node.id + '"]').hide();
				} else {
					ret = true;
				}
			} else {
		...
		if (opts.href){
			if(opts.type == 'frame') {
				if (!state.isLoaded){
					...
					$(target).empty();
					//_load(opts.content);
					//_load('<iframe src="' + opts.href + '" frameborder="0" style="border:0;width:100%;height:100%;"></iframe>');
					//$($(target).find('iframe')[0])
					var frame = $('<iframe src="' + opts.href + '" frameborder="0" style="border:0;width:100%;height:100%;"></iframe>').appendTo($(target));
					
//					frame[0].contentWindow.parenter = opts.args;
					
					/*
					if (frame[0].attachEvent) {
						frame[0].attachEvent('onreadystatechange', function() {
							alert('ready:' + frame[0].readyState);
						});
					}
					
					if (frame[0].addEventListener) {
						frame[0].addEventListener('readystatechange', function() {
							alert('ready:parent');
						}, false);
					}
					*/
					...
					*/
					frame.on('load', function(){
						var contentWin = frame[0].contentWindow;
						contentWin.args = opts.args;
						if (contentWin.__init && typeof contentWin.__init == 'function') {
							contentWin.__init();
						}
						//opts.onLoad.call(target, contentWin);
						opts.onLoad.call(contentWin);
					});
//					opts.onLoad.apply(target, arguments);
	...
	function openPanel(target, forceOpen){
		...
		if (typeof forceOpen == 'boolean' && forceOpen != true){
		...
		if(typeof forceOpen == 'object') {
			if(opts.type == 'frame') {
				//for(var p in forceOpen) {
				//	$(target).find('iframe')[0].contentWindow[p] = forceOpen[p];
				//}
				//$(target).find('iframe')[0].contentWindow = $.extend($(target).find('iframe')[0].contentWindow, forceOpen);
				//$($(target).find('iframe')[0].contentWindow).on('load', function(){
				//	alert(1);
				//});
//				$($(target).find('iframe')[0])
//				.onload = function() {
...
//			}
	...
	$.fn.panel.methods = {
		...
		frameWindow : function(jq) {
			var frame = $(jq[0]).find('iframe')[0];
			if (frame) {
	...
	$.fn.panel.parseOptions = function(target){
		...
		return $.extend({}, $.parser.parseOptions(target, ['id','width','height','left','top',
		        'title','iconCls','cls','headerCls','bodyCls','tools','href','type',
...
//		}
	...
	$.fn.panel.defaults = {
		...
		style: {},
		type : 'ajax',
		args : null,	//type为frame时给子窗口传递的参数
		href: null,
		cache: true,
		fit: false,
		border: true,
...
//		}
	...
	function init(container){
		...
		function _add(cc){
			...
			cc.children('div').each(function(){
				...
				var r = opts.region;
				//增加选择，让布局的每个子面板的位置可通过id设置，而不需要设置data-options参数
				if (!r){
					var id = $(this).attr("id");
					var regions = ["north","south","east","west","center"];
					for (var i=0;i<5;i++){
						if (layoutOpts[regions[i]] && id == layoutOpts[regions[i]].id)
							r = regions[i];							
					}
				}
	...
	function addPanel(container, param, el){
		...
		if (opts[dir]){
			var width = opts[dir].width;
			var height = opts[dir].height;
			if (typeof width == 'number')
				width = width + "px";
			if (typeof height == 'number')
				height = height + "px";
			pp.css("width",width);
			pp.css("height",height);
		}
		...
		}, param, opts[dir]||{})); //此处作了修改，参数扩展时添加了opts[dir]，即js中设定的参数。这样，可通过js设置参数，无需data-options。		
			...
			if (typeof opts.toolbar == 'string'){
				...
				for(var i=0; i<opts.toolbar.length; i++) {
					...
					if (btn == '-') {
						 ...
						 * chenlia-2013/09/03
						 */
						$('<td><div class="datagrid-btn-fill"></div></td>').appendTo(tr);
					} else {
						var td = $('<td></td>').appendTo(tr);
						/**
						 * 增加xtype使得工具条支持其他控件
						 * chenlia-2013/09/03
						 */
						if (!btn.xtype || btn.xtype === 'linkbutton') {
							var tool = $('<a href="javascript:void(0)"></a>').appendTo(td);
							tool[0].onclick = eval(btn.handler || function(){});
							tool.linkbutton($.extend({}, btn, {
								plain:true
							...
							var tool = $('<input />').appendTo(td);
							eval('tool.' + btn.xtype + '(btn);');
						}
					}
				}
			}
...
//			return parseInt(tr.attr('datagrid-row-index'));
	 ...
	 * chenlia-2013/09/03
	 */
	function fixToolbarSize(target) {
		var panel = $.data(target, 'datagrid').panel;
		var btnfill = panel.find('div.datagrid-btn-fill:first');
		if (btnfill.length == 1) {
			btnfill.width(0);
			var tablewidth = panel.find('.datagrid-toolbar>table').width();
			var toolbarwidth = panel.find('.datagrid-toolbar').width();
			btnfill.width(toolbarwidth - tablewidth);
		}
	};
...
//		bindRowEvents(target);
	 ...
	 * chenlia-2013/09/11
	 */
	function unSelectRecord(target, idValue) {
		var opts = $.data(target, 'datagrid').options;
		...
		if (opts.idField){
			...
			if (index >= 0){
				...
				//从selectedRows中去除对应数据
				removeArrayItem(selectedRows, opts.idField, idValue);
			}
		}
	};
...
//		bindRowEvents(target);
		...
		view.refreshRowsBelowIndex(target, param.index, data.rows.length);		
	}
	...
	//增加的datagrid方法，“moveRow”的实现
	function moveRow(target, index, targetIndex){
	...
	var editors = {
		...
		combodept : {
			init : function(container, options) {
				var combo = $('<input />').appendTo(container);
				combo.combodept(options);
				return combo;
			},
			destory : function(target) {
				$(target).combodept('destpry');
			},
			getValue : function(target) {
				return $(target).combodept('getValue');
			},
			setValue : function(target, value) {
				$(target).combodept('setValue', value);
			},
			resize : function(target, width) {
				$(target).combodept('resize', width);
			}
		}
	};
...
//			}
		...
		//新增unSelectRecord方法	chenlia-2013/09/11
		unSelectRecord: function(jq, id){
			return jq.each(function(){
				unSelectRecord(this, id);
			});
		},
		...
		moveRow: function(jq, param){
			return jq.each(function(){
				moveRow(this, param.index, param.dest);
			});
		},
...
//						style += 'text-align:' + (col.align || 'left') + ';';
		...
		refreshRowsBelowIndex: function(target, rowIndex, rowNum){
			for (var i=rowIndex; i<rowNum;i++)
				this.updateRow.call(this, target, i, {});
		}, 
...
//						tr.find('div.datagrid-cell-rownumber').html(i);
			...
			opts.view.refreshRowsBelowIndex(target, index, data.rows.length);
		},
		...
		onAfterRender: function(target){
			...
			if (opts.emptyMsg && !$(target).datagrid('getRows').length){  
