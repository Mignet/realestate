...
SWFUpload.prototype.getFlashHTML = function () {
    return ['<object id="', this.movieName, '" type="application/x-shockwave-flash"  wmode="transparent" data="', this.settings.flash_url, '" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload">', '<param name="wmode" value="', this.settings.button_window_mode, '" />', '<param name="movie" value="', this.settings.flash_url, '" />', '<param name="quality" value="high" />', '<param name="menu" value="false" />', '<param name="allowScriptAccess" value="always" />', '<param name="flashvars" value="' + this.getFlashVars() + '" />', "</object>"].join("");
};
...
(function (c) {
    var b = {
        init: function (d, e) {
            return this.each(function () {
                ...
                var j = c.extend({
                    id: n.attr("id"),
                    swf: "../../extend/uploader/uploadify.swf",
    ...
    var a = {
        onDialogOpen: function () {
            var d = this.settings;
            this.queueData.errorMsg = "有些文件没有被添加到列表中:";
        ...
        onSelect: function (h) {
            ...
            for (var g in this.queueData.files) {
                ...
                if (f.uploaded != true && f.name == h.name) {
                    var e = confirm('名为 "' + h.name + '" 的文件已经在队列中.\n您是否要替换?');
                    if (!e) {//不替换
                        ...
                        c("#" + f.id).remove();
						c("#" + f.id+'_0').parent().parent().remove();
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                switch (g) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 超过大小限制 (' + e.fileSizeLimit + ").";
                        break;
