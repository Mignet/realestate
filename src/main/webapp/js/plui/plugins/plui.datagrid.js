...
//				var height = Math.max(tr1.height(), tr2.height());
				//解决datagrid合并后高度问题，暂时去最小值，找到合适方法后再修改。
				var height = Math.min(tr1.height(), tr2.height());
...
//		bindRowEvents(target);
	...
	function checkAll(target, notSelect){
		...
		var dc = state.dc;
		var hck = dc.header1.add(dc.header2).find('input[type=checkbox]:enabled');
		var bck = opts.finder.getTr(target, '', 'allbody').find('div.datagrid-cell-check input[type=checkbox]:enabled');
...
//						style += 'text-align:' + (col.align || 'left') + ';';
					...
					if (col.checkbox){
						var formatText = '';
						if (col.formatter)
							formatText = col.formatter(value, rowData, rowIndex);						
						cc.push('<input type="checkbox" name="' + field + '" ' + formatText + ' value="' + (value!=undefined ? value : '') + '"/>');
