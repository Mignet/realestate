/*********************************************************************************
*文  件  名  称: mort-shift-acc.js
*功  能  描  述: 一般抵押权转移登记申请表js
...
$(function() {
	...
	mortgager = $('#table_transferor').datagrid({
		...
		height:200,
		// 表格数据来源
		url :ctx+"/mortgage/morsetup!getMortapp.action?time="+new Date()+"&proc_id="+proc_id+"&apptype=063005",
		// 表格每列宽度自动适应表格总宽度
		autoRowHeight : true,
		fitColumns : false,
		// 去掉边框
	...
	***********************************************************************************/
	...
	//创建抵押权人信息表
	 userDataGrid = $('#table_user').datagrid({
		...
		height:200,
		// 表格数据来源
		url :ctx+"/mortgage/morsetup!getMortapp.action?time="+new Date()+"&proc_id="+proc_id+"&apptype=063006",
		// 表格每列宽度自动适应表格总宽度
		autoRowHeight : true,
		fitColumns : false,
		// 去掉边框
	...
	***********************************************************************************/
	function doAdd() {
		var hol_rel = "063006";						//默认权利人关系为抵押权受让方
		openInTopWindow({
			// 窗口元素的id
			id : 'add_user_win',
			// 窗口iframe的src
