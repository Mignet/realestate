...
***********************************************************************************/
$(function() {
	//$(".body_set").css({"height":"500px","overflow-y":"scroll"});
	...
	mortgager = $('#table_transferor').datagrid({
		...
		height:200,
		// 表格数据来源
		url :ctx+"/mortgage/morsetup!getMortapp.action?time="+new Date()+"&proc_id="+proc_id+"&apptype=063003",
		// 表格每列宽度自动适应表格总宽度
		autoRowHeight : true,
		fitColumns : false,
		// 去掉边框
	...
	***********************************************************************************/
	...
	//创建抵押权人信息表
	 userDataGrid = $('#table_user').datagrid({
		...
		height:200,
		// 表格数据来源
		url :ctx+"/mortgage/morsetup!getMortapp.action?time="+new Date()+"&proc_id="+proc_id+"&apptype=063004",
		// 表格每列宽度自动适应表格总宽度
		autoRowHeight : true,
		fitColumns : false,
		// 去掉边框
...
function getMortMess(){
	$.ajax({
	    dataType: 'json',
		url:ctx+"/mortgage/morsetup!getMortById.action?time="+new Date()+"&proc_id="+proc_id,
		success:function(data){
		 	if(data){
