...
window.onload = function(){
	...
	//设置表头
	$("#title").text("登记中心办文质量检查月汇总表(0"+(new Date().getMonth()+1)+"月 )");
}
/**********************************************************************************
...
function initStatisticsData(){
		$.ajax({
			url:ctx+"/qualityinspection/qualityinspection!getCurMonthStatistics.action",
			type:"get",
			dataType:"json",
...
//				}
				for(var i=0;i<data.length;i++){
					append_str+="<tr><td colspan='2'>"+data[i].DEPT_NAME+"</td><td>"+data[i].DEPT_ALL_TOTAL+"</td><td>"+data[i].DEPT_ALL_ERROR+"</td><td>"+data[i].ONW_INIT_TOTAL+"</td><td>"+data[i].ONW_INIT_NUM+
					...
					"</td><td>"+data[i].TARGET_NUM+"</td><td>"+data[i].ACTUAL_NUM+"</td></tr>";
				}
...
function queryBydate(){
	...
	$("#title").text("登记中心办文质量检查月汇总表("+arr[1]+"月 )");
	//alert(start_date+end_date);
	$.ajax({
		url:ctx+"/qualityinspection/qualityinspection!getCurMonthStatistics.action",
		type:"get",
		dataType:"json",
		data:{"start_date":start_date,"end_date":end_date,"reg_station":reg_station},
		success:function(data){
			//alert($.toJSON(data));
			if(data.failed){
				top.$.messager.alert('提示',"该时间段没有抽检业任务！",'info',null);
			...
			var append_str = '<tr><td colspan="2" rowspan="2">科室/类型</td><td colspan="2">窗口收文</td><td colspan="2">初始登记</td><td colspan="2">二级转移</td><td colspan="2">三级转移</td><td colspan="2">预售抵押</td><td colspan="2">现售房抵押</td><td colspan="2">变更及其它</td><td colspan="2">安居房换证</td><td colspan="2">预售合同备案</td><td colspan="2">产权综合办文</td><td colspan="2">两规登记</td><td colspan="2">其它登记</td><td colspan="2">合计</td>'+
			'</tr><tr>'+
			'<td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td >抽检量</td><td >差错量</td><td>目标抽检量</td><td>实际抽检量</td>'+
  			'</tr>';
//			for(var i=0;i<data.length;i++){
//				if(i==0){
//					append_str="<tr><td colspan='2'>"+data[i].DEPT_NAME+"</td><td>"+data[i].TOTAL+"</td><td>"+data[i].ERROR_NUM+"</td>";
