...
window.onload = function(){
	...
	//设置表头
	$("#title").text("科室办文质量检查情况统计表(0"+(new Date().getMonth()+1)+"月 )");
...
function queryBydate(){
	...
	//修改统计表标题
	var dept_name = $("#dept").combodept("getText");
	//alert();
	var arr=start_date.split('-');
	$("#title").text(dept_name+"科室办文质量检查情况统计表("+arr[1]+"月 )");
	
	//alert(start_date+end_date);
	var dept = $("#dept").combodept("getValue");
	//alert(dept);
	//return;
	$.ajax({
		url:ctx+"/qualityinspection/qualityinspection!getDaydetailStatistics.action",
		type:"get",
		dataType:"json",
		data:{"start_date":start_date,"end_date":end_date,"dept_id":dept},
		success:function(data){
			//alert($.toJSON(data));
			if(data.failed){
				top.$.messager.alert('提示',"该时间段没有抽检业任务！",'info',null);
