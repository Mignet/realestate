...
window.onload = function(){
	...
	//设置表头
	$("#title").text("办文质量检查情况统计表(0"+(new Date().getMonth()+1)+"月 )");
...
function queryBydate(){
	...
	//修改统计表标题
	var location_name = $("#location").combo("getText");
	//alert();
	var arr=start_date.split('-');
	$("#title").text(location_name+"办文质量检查情况统计表("+arr[1]+"月 )");
...
function doPrintPreview(){
	...
	// var eprnstr="<!--endprint"+oper+"-->";//设置打印结束区域
	 var prnhtml=document.getElementById("print").innerHTML; //从开始代码向后取html
