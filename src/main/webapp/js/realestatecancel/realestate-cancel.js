...
function  saveReason(){
	...
	{
		var regtype=$('#table_house').datagrid('getRows')[0].TYPE;
		if(regtype==enumdata.parcel)
		{
			url= ctx+"/personal-office/land-use-right/acceptance!saveUseright.action?time="+new Date()+"&proc_id="+proc_id;
		}
		else
		{
...
function getReason(){
	...
	if($('#table_house').datagrid('getRows').length>0)
	{
		...
		if(regtype==enumdata.parcel)
		{
			url= ctx+"/personal-office/land-use-right/acceptance!getUseright.action?time="+new Date()+"&proc_id="+proc_id;
		}
		else
		{
	...
	$.ajax({
		    dataType: 'json',
			url:url,//ctx+"/mortgage/morsetup!getRegInfo.action?time="+new Date()+"&proc_id="+proc_id,
			success:function(data)
			{
			 	if(data)
			 	{
...
function validate(v_flag){
	...
	if(proc_node==state1.string0){
		message = "注销原因！";
		var REASON = new tt.Field(message,"REASON");  
	    //非空检验
	    tt.vf.req.add(REASON);
