...
var change_dict_class_code = '060';					//变更变更数据项字典代码(获取变更字典代码  扩展  直接在字典项中添加就行)
...
change_data_item.HOL_POSITION='060018';			    //权利人份额
change_data_item.HOL_ADDR='060019';			        //权利人地址
change_data_item.HOL_TELL='060020';			        //权利人电话
change_data_item.HOL_POSTCODE='060021';			    //权利人邮编
change_data_item.HOL_LEGAL_PERSON='060022';			//权利人之法人
change_data_item.HOL_LEGAL_CER_ID='060023';			//权利人之法人身份证
change_data_item.HOL_AGENT='060024';			    //权利人之代理人
change_data_item.HOL_AGENT_CER_ID='060025';			//权利人之代理人身份证
change_data_item.HOL_AGENT_TELL='060026';			//权利人之代理人电话
change_data_item.HOL_TYPE='060027';			        //权利人类型
change_data_item.DEPART_TYPE='060028';			    //单位性质
change_data_item.HOL_REL='060029';			        //权利人种类
change_data_item.CER_TYPE='060030';			        //证件类型
		...
		onLoadSuccess : function(data) {
			...
			if(proc_node=='受理'){
				...
				//其它情况下保存到改变后的权利人中	
					...
					//判断权利人(名称、身份证号)是否已经改变 如果改变则在变更变更记录表中插入一行数据
	...
	$('#div_change_detail').datagrid({
		...
		columns : [ [
		     		...
		     		onAfterEdit:function(index,row){     
		     	        row.editing = false;     
		     	        $('#div_change_detail').datagrid('refreshRow', index);     
		     	    },
		     		onLoadSuccess : function(data) {
...
function setState(proc_node) {
	...
	}else if(proc_node == state1.string1){
		$("#REG_STATION").combo('disable');
		$("#REG_TYPE").combo('disable');
...
function  saveExcursus(){
	...
	//var result = validate(); 
		 ...
		 $.ajax({
		   		dataType:'json',
		   		url:ctx+"/houseownership/correction!saveExcursus.action?time="+new Date()+"&proc_id="+proc_id,
		   		contentType:"application/x-www-form-urlencoded; charset=GBK",
		   		//表单的序列化操作
...
//	 if(endEditing()){
//         $('#div_change_detail').datagrid('selectRow',rowIndex).datagrid('beginEdit',rowIndex)//其中beginEdit方法为datagrid的方法，具体可以参看api
//         editIndex = rowIndex;//给editIndex对象赋值，index为当前行的索引
//         
...
//     }else {
//           
//    	 $('#div_change_detail').datagrid('selectRow',edtiIndex);
//     
//     
//     }
...
function endEditing()
{
   ...
   if($('#div_change_detail').datagrid('validateRow',editIndex)) {
       $('#div_change_detail').datagrid('endEdit',editIndex);//当前行编辑事件取消
...
function saveChangeInfo() {
	 ...
	 //alert($.toJSON(rows));
	 if(rows.length == 0) {
		 top.$.messager.alert('警告', '无需保存,未发生数据改变！', 'warn', function(){}); 
		 return;
	 }
     for ( var i = 0; i < rows.length; i++) {
		...
		//验证 变更后数据项是否非空填写  只验证新增和更新的
		if(inserted){
			for(var i=0;i<inserted.length;i++){
				if(inserted[i].NEW_DATA.length==0){
					top.$.messager.alert('提示', '变更后数据不能为空！', 'info',
							function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<data.length;i++){
			    ...
			    //显示权利人信息
				}else if(code == change_data_item.HOL_NAME || code == change_data_item.HOL_CER_NO 
						|| code == change_data_item.HOL_ADDR || code == change_data_item.HOL_LEGAL_PERSON
						|| code == change_data_item.HOL_POSITION  || code == change_data_item.HOL_TYPE 
						|| code == change_data_item.LOCATION_AREA || code == change_data_item.DEPART_TYPE
						|| code == change_data_item.HOL_REL || code == change_data_item.CER_TYPE){
					//不需要显示的信息
					if(code == change_data_item.LOCATION_AREA){
						continue;
					}
					if(old_data.reg_unit_type == reg_unit_type.PARCEL){
						continue;
					...
					if(count%dataColumn==0){
						str+='<tr><td><input id="'+data[i].value+'"  type="checkbox" onclick="cbxClick_hol(this)"/>'+
						'<label for="'+data[i].value+'" id="lbl_'+data[i].value+'">'+data[i].name+'</label></td>';
					}else if(count%dataColumn==5){
						str+='<td><input id="'+data[i].value+'"  type="checkbox" onclick="cbxClick_hol(this)"/>'+
						'<label for="'+data[i].value+'" id="lbl_'+data[i].value+'">'+data[i].name+'</label></td></tr>';
					}else{
						str+='<td><input id="'+data[i].value+'"  type="checkbox" onclick="cbxClick_hol(this)"/>'+
						'<label for="'+data[i].value+'" id="lbl_'+data[i].value+'">'+data[i].name+'</label></td>';
...
/**********************************************************************************
*函数名称: 判断权利人(名称、身份证号)是否已经改变 如果改变则在变更变更记录表中插入一行数据  
*功能说明: 判断权利人(名称、身份证号)是否已经改变 如果改变则在变更变更记录表中插入一行数据  
...
*参数说明: code(字典项code) old_data(变更前数据) new_data(变更后数据)
*返 回 值: 
*函数作者: Joyon
*创建日期: 2014-03-01
*修改历史: 
...
function validate(v_flag){
	...
	if(proc_node==state1.string1){
		...
		if(!v_flag){
			...
			//变更内容数据项校验
			 tmpData = $('#div_change_detail').datagrid('getChanges');
			 if(tmpData.length>0){
				 var flag= 0 ;//用来确认 是否用户已经点击放弃保存  未点击  代表是在外面调用     返回false
				 message = '数据项未保存！请先保存后再进行操作！';
				 if(flag){
