		...
		columns : [ [
		     		{
		     			...
		     			title : '代理人证件类型',
		     			field : 'AGENT_CER_TYPE',formatter : dicts.format.app_cer_type_format
		     		},{
		     			title : '代理人身份证号',
		     			field : 'AGENT_CER'
		     		}, {
...
function setState(proc_node) {
	...
	if(proc_node == state1.string0){
		$("#div_exccursus").css("display","none");
...
function saveRegInfo() {
	...
	var location_reg_unit = $("#LOCATION_REG_UNIT").combo('getValue');
	 ...
	 var EXCURSUS = $("#EXCURSUS").val();
	  $.ajax({
	   		dataType:'json',
	   		url:ctx+"/houseownership/shiftreg!saveRegInfo.action?time="+new Date()+"&proc_id="+proc_id,
	   		contentType:"application/x-www-form-urlencoded; charset=GBK",
	   		//表单的序列化操作
	   		data:{"get_mode":get_mode,"reg_value":reg_value,"reg_station":reg_station,"house_attr":house_attr,"excursus":EXCURSUS,"location_reg_unit":location_reg_unit},
	   		success:function(data){
			 	if(data){
...
function getPreRegMess(){
	$.ajax({
		    dataType: 'json',
			url:ctx+"/houseownership/shiftreg!getRegInfo.action?time="+new Date()+"&proc_id="+proc_id,
			success:function(data){
			 	if(data){
			 		...
			 		if(data.REG_VALUE){
			 			$("#REG_VALUE").numberbox('setValue', data.REG_VALUE);
			 		}
