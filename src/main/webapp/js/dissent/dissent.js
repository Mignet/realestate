...
*修改历史: 
***********************************************************************************/
function  saveDisitem(){
	var disitem = $("#DISS_ITEM").val();
	//var selectedrow = $('#table_house').datagrid('getSelected');
	var url;
	if($('#table_house').datagrid('getRows').length>0)
	{
			url= ctx+"/houseownership/initialreg!saveDemurrer.action?time="+new Date()+"&proc_id="+proc_id;
	}
	var result = true; 
		 $.ajax({
		   		dataType:'json',
...
function setState(proc_node) {
	...
	if(proc_node == state1.string1){
		...
		$("#DISS_ITEM").attr("disabled", "disabled");
...
function submit(){	
	...
	if(proc_node == state1.string0)
	{
		var ss= $("#REG_TYPE").combodict('getValue');
		//$("#REG_STATION").combodict("setValue");
		if($("#REG_TYPE").combodict("getValue")!=enumdata.UNDEMURRER)
 		{
			saveDisitem();
 			//$("#DISS_ITEM").attr("disabled", "disabled");
 		}
...
*修改历史: 
***********************************************************************************/
function getDisitem(result){
	var url;
	if(result==false)
	{
		url= ctx+"/houseownership/initialreg!getDemurrer.action?time="+new Date()+"&proc_id="+proc_id;
	}
	else
	{
		url= ctx+"/houseownership/initialreg!getDisItem.action?time="+new Date()+"&reg_code="+result;
	}	
	$.ajax({
		    dataType: 'json',
			url:url,
	...
	$.ajax({
			...
			success:function(data){
			 	if(data){
			 		...
			 		if(data.REG_TYPE==enumdata.UNDEMURRER)
			 		{
			 			$("#DISS_ITEM").attr("disabled", "disabled");
			 			getDisitem($("#REG_CODE").val());
			 		}
			 		if(data.MORT_MODE){
