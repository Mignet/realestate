function init() {
	var form = $('#add_building_form').tableform({
		errorCode: 700,
		//表单元素表格的列数（通常设为两列）
		colnum: 2,
		//表单控件标题列宽
		...
		url: 'building!saveBuilding.action',
		dataType: 'json',
		success: function(data){
			if (data.success) {
				top.$.messager.alert('新增建筑提示',data.tipMessage,'info',function(){
					args.buildingDataGrid.datagrid('reload');
					closeInTopWindow('add_building_win');
				});				
			} else {
				top.$.messager.alert('新增建筑错误',data.errorMessage,'error');
			}
		}
	});
};
