	...
	***********************************************************************************/
		function dowatch(button){
			...
			openInTopWindow({
				...
				id : 'add_user_win',
				// 窗口iframe的src
				src : ctx+'/jsp/common/registerbook/registeruse.jsp?time='+new Date(),
				// 关闭时是否销毁窗口。不销毁的话，每次打开窗口都会添加一个新窗口元素。
				destroy : true,
				// 窗口标题
				title : '房地产信息',
...
function getPreRegMess(){
	$.ajax({
		    dataType: 'json',
			url:ctx+"/reissue/reissue!regRegInfo.action?time="+new Date()+"&proc_id="+proc_id,
			success:function(data){
			 	if(data){
...
function validate(v_flag){
	...
	if(proc_node==state1.string1){
		message = "房地产证不能为空！";
		var EXCURSUS = new tt.Field(message,"EXCURSUS");  
	    //非空检验
	    tt.vf.req.add(EXCURSUS);
...
*修改历史: 
***********************************************************************************/
function saveBkInfoToBus(){
	$.ajax({
		url:ctx+"/reissue/reissue!saveBkInfoToBus.action",
		dataType : 'json',
		type : 'post',
